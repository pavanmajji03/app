...
// Matches the shape returned by build_report() via pollAnalysis_CreatorAnalysis
...
function CreatorCard({ item, selected, onToggle, color, loadingReport }: {
  item: WatchlistCampaign; selected: boolean; onToggle: () => void; color: string; loadingReport: boolean;
}) {
  const { palette } = useTheme();
  const progress = pct(item.raised_amount, item.target_amount);
    ...
    >
      <div className="relative h-36 overflow-hidden" style={{ backgroundColor: `${color}18` }}>
        ...
        <div className="absolute top-3 left-3">
          {selected
            ? <CheckCircle2 size={20} style={{ color }} fill={color} />
            : <Circle size={20} style={{ color: palette.textSubtle }} />}
        </div>
        ...
        <div className="absolute top-3 right-3">
          <div className="flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: `${scoreColor}18`, color: scoreColor, border: `1px solid ${scoreColor}30` }}>
            <Star size={10} fill={scoreColor} strokeWidth={0} /> {item.ai_score}
          </div>
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="mb-2">
          <div className="flex justify-between text-xs mb-1">
            <span style={{ color: palette.textMuted }}>Funded</span>
        ...
        <div className="flex items-center justify-between mt-2">
          <div>
            <p className="text-xs" style={{ color: palette.textMuted }}>Return Range</p>
            <p className="text-xs font-bold" style={{ color: palette.success }}>{item.return_low}% – {item.return_high}%</p>
          </div>
          {item.genres[0] && (
...
function CreatorCol({ campaign, color, children }: { campaign: WatchlistCampaign; color: string; children: React.ReactNode }) {
  const { palette } = useTheme();
  return (
...
function CompareView({ selected, campaigns, reports, loadingReports, onBack }: {
  selected: string[]; campaigns: WatchlistCampaign[];
  ...
  // Chart: Return Scenarios
  const returnData = [
    { scenario: 'Low', ...Object.fromEntries(creators.map(c => [c.creator_name, c.return_low])) },
    { scenario: 'Base', ...Object.fromEntries(creators.map(c => [c.creator_name, c.return_base])) },
    { scenario: 'High', ...Object.fromEntries(creators.map(c => [c.creator_name, c.return_high])) },
  ];

  // Chart: View Forecast (base scenario)
    ...
    <div style={{ backgroundColor: palette.bg, minHeight: '100vh' }}>
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <button onClick={onBack} className="flex items-center gap-2 text-sm mb-6 px-3 py-1.5 rounded-lg" style={{ color: palette.textMuted, backgroundColor: palette.surface, border: `1px solid ${palette.border}` }}>
          <ChevronLeft size={14} /> Back to Watchlist
        </button>
        ...
        <p className="text-sm mb-8" style={{ color: palette.textMuted }}>
          Side-by-side analysis of {n} creator{n > 1 ? 's' : ''} — combining campaign data and stored AI analysis
          ...
          {creators.map((c, i) => {
            ...
            return (
              <div key={c.campaign_id} className="rounded-2xl p-4 text-center" style={{ backgroundColor: palette.surface, border: `2px solid ${COLORS[i]}40` }}>
                ...
                <div className="text-2xl font-black mb-0.5" style={{ color: scoreColor }}>{c.ai_score}</div>
        ...
        {/* ── View Forecast (from stored report) ── */}
        {hasForecast && (
          <Section title="View Forecast — Base Scenario (Millions)" icon={TrendingUp}>
            ...
            {/* Low / Base / High table per creator */}
            <div className="mt-4">
              <ColGrid count={n}>
                {creators.map((c, i) => {
                  const fc = reports[c.campaign_id]?.view_forecast_millions;
                  if (!fc) return <div key={c.campaign_id} className="rounded-xl p-3 text-center" style={{ backgroundColor: palette.surface }}><p className="text-xs" style={{ color: palette.textSubtle }}>No forecast data</p></div>;
                  const fmtM = (v?: number) => v != null ? `${v}M` : '—';
                  return (
                    <CreatorCol key={c.campaign_id} campaign={c} color={COLORS[i]}>
                      {[
                        { label: '30d Low / Base / High', values: `${fmtM(fc.low_conservative?.['30d'])} / ${fmtM(fc.base_expected?.['30d'])} / ${fmtM(fc.high_optimistic?.['30d'])}` },
                        { label: '90d Low / Base / High', values: `${fmtM(fc.low_conservative?.['90d'])} / ${fmtM(fc.base_expected?.['90d'])} / ${fmtM(fc.high_optimistic?.['90d'])}` },
                        { label: '180d Low / Base / High', values: `${fmtM(fc.low_conservative?.['180d'])} / ${fmtM(fc.base_expected?.['180d'])} / ${fmtM(fc.high_optimistic?.['180d'])}` },
                      ].map(row => <MetricRow key={row.label} label={row.label} value={row.values} />)}
                    </CreatorCol>
                  );
                })}
              </ColGrid>
        ...
        {/* ── Revenue Scenarios (from stored report) ── */}
        ...
        {/* ── Return Scenarios chart (from campaign data) ── */}
        <Section title="Return Scenarios — Campaign (%)" icon={TrendingUp}>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={returnData} barGap={4} barCategoryGap="30%">
              <CartesianGrid strokeDasharray="3 3" stroke={`${palette.border}60`} vertical={false} />
              <XAxis dataKey="scenario" tick={{ fill: palette.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fill: palette.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={chartTooltipStyle} labelStyle={{ color: palette.text }} formatter={(v: any) => [`${v}%`, '']} />
              <Legend wrapperStyle={{ color: palette.textMuted, fontSize: 11, paddingTop: 8 }} />
              {creators.map((c, i) => <Bar key={c.campaign_id} dataKey={c.creator_name} fill={COLORS[i]} radius={[4, 4, 0, 0]} maxBarSize={48} />)}
            </BarChart>
          </ResponsiveContainer>
        </Section>

        {/* ── Campaign Terms ── */}
        <Section title="Campaign Terms" icon={Users}>
          <ColGrid count={n}>
            {creators.map((c, i) => {
              const progress = pct(c.raised_amount, c.target_amount);
              return (
                <CreatorCol key={c.campaign_id} campaign={c} color={COLORS[i]}>
                  <MetricRow label="Term" value={`${c.term_months} months`} />
                  <MetricRow label="Revenue Share" value={`${c.revenue_share_pct}%`} />
                  <MetricRow label="Target" value={`$${c.target_amount.toLocaleString()}`} />
                  <MetricRow label="Raised" value={`$${c.raised_amount.toLocaleString()}`} />
                  <MetricRow label="Backers" value={`${c.investor_count}`} />
                  <div className="mt-2">
                    <div className="flex justify-between text-xs mb-1">
                      <span style={{ color: palette.textMuted }}>Funded</span>
                      <span style={{ color: palette.text }}>{progress}%</span>
                    ...
                    <div className="h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: palette.surfaceAlt }}>
                      <div className="h-full rounded-full" style={{ width: `${progress}%`, backgroundColor: COLORS[i] }} />
                    </div>
                  </div>
                </CreatorCol>
              );
        ...
        {/* ── Risk Factors (from stored report) ── */}
        {creators.some(c => reports[c.campaign_id]?.risk_factor_analysis) && (
          <Section title="Risk Factor Analysis (from AI Report)" icon={Shield}>
            <ColGrid count={n}>
              {creators.map((c, i) => {
        ...
        {/* ── Social Signals (from stored report) ── */}
                  ...
                  <CreatorCol key={c.campaign_id} campaign={c} color={COLORS[i]}>
                    ...
                    {topics?.top_trending_topics && topics.top_trending_topics.length > 0 && (
                      <div className="mt-2 pt-2" style={{ borderTop: `1px solid ${palette.border}30` }}>
                        ...
                        {topics.momentum && <p className="text-xs mt-1" style={{ color: palette.textSubtle }}>Momentum: {topics.momentum}</p>}
                      </div>
                    )}
                  </CreatorCol>
                );
        ...
        {/* ── Recent Videos (from stored report) ── */}
        {creators.some(c => (reports[c.campaign_id]?.youtube_signals?.recent_videos_preview ?? []).length > 0) && (
          <Section title="Recent Videos" icon={PlayCircle}>
            <ColGrid count={n}>
              {creators.map((c, i) => {
                const videos = reports[c.campaign_id]?.youtube_signals?.recent_videos_preview ?? [];
                return (
                  <CreatorCol key={c.campaign_id} campaign={c} color={COLORS[i]}>
                    {videos.length === 0
                      ? <p className="text-xs" style={{ color: palette.textSubtle }}>No video data</p>
                      : videos.map((v, vi) => (
                        <div key={vi} className="flex items-center gap-2 mb-2 last:mb-0">
                          <div className="w-12 h-8 rounded flex-shrink-0 overflow-hidden" style={{ backgroundColor: `${COLORS[i]}18` }}>
                            {v.thumbnail
                              ? <img src={v.thumbnail} alt={v.title} className="w-full h-full object-cover" />
                              : <div className="w-full h-full flex items-center justify-center"><PlayCircle size={12} style={{ color: COLORS[i] }} /></div>}
                          </div>
                          <div className="min-w-0">
                            <p className="text-xs truncate" style={{ color: palette.text }}>{v.title}</p>
        ...
        {/* ── AI Assessment (from stored report) ── */}
                ...
                return (
                  <CreatorCol key={c.campaign_id} campaign={c} color={COLORS[i]}>
                    ...
                    {a?.confidence_score != null && (
                      <p className="text-xs mt-1" style={{ color: palette.textSubtle }}>Confidence: {a.confidence_score}/100</p>
                    )}
                  </CreatorCol>
                );
              })}
        ...
        {/* Loading placeholder when no report sections loaded yet */}
        {loadingReports && !hasForecast && !creators.some(c => reports[c.campaign_id]?.risk_factor_analysis) && (
          <div className="flex items-center justify-center gap-2 py-10" style={{ color: palette.textMuted }}>
            ...
            <span className="text-sm">Fetching full AI reports from stored data…</span>
          </div>
        )}
      </div>
    </div>
...
export function Watchlist() {
  const { palette } = useTheme();
  const [campaigns, setCampaigns] = useState<WatchlistCampaign[]>([]);
  ...
  useEffect(() => {
    callApi<WatchlistCampaign[]>('listLiveCampaigns_Marketplace')
      .then(res => setCampaigns(res.data ?? []))
      .catch(() => setCampaigns([]))
      .finally(() => setLoading(false));
  }, []);

  // Eagerly fetch reports for selected creators so compare loads instantly
  const fetchReport = useCallback(async (campaign: WatchlistCampaign) => {
  ...
  const toggleSelect = useCallback((id: string) => {
    setSelected(prev => {
      ...
      const next = [...prev, id];
      // Start fetching the report for newly selected creator
  ...
  return (
    <div style={{ backgroundColor: palette.bg, minHeight: '100vh' }}>
      <div className="max-w-7xl mx-auto px-6 py-10 pb-28">
        ...
        <div className="mb-6">
          <h1 className="text-3xl font-bold mb-1" style={{ color: palette.text }}>Watchlist</h1>
          <p className="text-sm" style={{ color: palette.textMuted }}>
            Find creators with active campaigns and compare their full AI reports side by side.
          </p>
        </div>
        ...
        {!loading && (
          <p className="text-sm mb-5" style={{ color: palette.textMuted }}>
            <span style={{ color: palette.text }}>{filtered.length}</span> creator{filtered.length !== 1 ? 's' : ''} with active campaigns
        ...
        {loading ? (
          <div className="flex items-center justify-center gap-2 py-20" style={{ color: palette.textMuted }}>
            <Loader2 size={18} className="animate-spin" /><span>Loading campaigns…</span>
          </div>
        ) : filtered.length > 0 ? (
          ...
          <div className="text-center py-20">
            <p className="text-lg font-semibold mb-2" style={{ color: palette.text }}>
              {campaigns.length === 0 ? 'No active campaigns yet' : 'No creators match your search'}
            </p>
